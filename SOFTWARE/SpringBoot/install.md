...
## Inicijalizacija Keycloaka

Pokrećemo keycloak i postgres:

```sh
docker compose -f docker-compose-infrastructure.yml up
```

ili umjesto `docker compose` možemo koristiti i `docker-compose`.

Pokrenuto je kada se ispiše sljedeće:

```sh
...
keycloak_1    | 06:51:07,954 INFO  [org.jboss.as] (Controller Boot Thread) WFLYSRV0051: Admin console listening on http://127.0.0.1:9990
```

Nakon toga u drugom terminalu (comamnd prompt) pokrenuti inicijalizaciju (kopiranje i izvršavanje skripte koja stvara korisnike):

```sh
docker cp init.sh rest-server_keycloak_1:/tmp/init.sh 
docker exec rest-server_keycloak_1 /bin/bash -c "source /tmp/init.sh"
```

Na ispisu će se ispisati parametri koje potrebno staviti u `application.properties` u Spring Boot:
...
keycloak.credentials.secret=e7f4f3e4-7503-467d-bb8e-e0463ba59020
keycloak.use-resource-role-mappings=true
keycloak.bearer-only=true
```

### Zaustavljanje keycloaka
...
### Brisanje keycloak konfiguracije

U novom terminalu pokrenuti:

```sh
docker volume rm rest-keycloak-fake_postgresql_data
```
...
Kopirati parametre koje je ispisala skripta u `rest-keycloak-fake/src/main/resources/application properties`.

### Pokretanje REST aplikacije
...
![Slijedni dijagram](sequenceDiagram.png)
...
```sh
curl -v -X GET 'http://localhost:8080/keys' \
--header "Authorization: bearer $keycloak_access"
...
```sh
curl -X GET 'http://localhost:8080/scene' \
--header "Authorization: bearer $keycloak_access"
...
### Dohvaćanje jedne scene

```sh
curl -X GET 'http://localhost:8080/scene/scena2' \
--header "Authorization: bearer $keycloak_access"
